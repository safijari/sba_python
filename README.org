...
** Installation from Source
   ...
   System dependencies:
   - Linux: =libeigen3-dev=, =libsuitesparse-dev=, =libopenblas-dev=, =liblapack-dev=
   - macOS: =eigen=, =suite-sparse=, =openblas=, =lapack= (install via Homebrew)
...
** GitHub Actions CI/CD
   ...
   - Supports Linux (x86_64) and macOS (x86_64 and arm64)
   - Automatically creates releases when tags starting with =v= are pushed
   - Uploads built wheels to GitHub releases
